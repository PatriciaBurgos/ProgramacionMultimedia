...
public class World {
		...
		while (y < WORLD_HEIGHT - WORLD_WIDTH / 2) {
			...
			if (rand.nextFloat() > 0.2f) {
				...
				coins.add(coin);
                                ...
                                Pig pig = new Pig(platform.position.x + rand.nextFloat(), platform.position.y + Pig.PIG_HEIGHT
					+ rand.nextFloat() * 3);
				pigs.add(pig);
			}
	...
	private void updateCoins (float deltaTime) {
        ...
        private void updatePigs (float deltaTime) {
		int len = pigs.size();
		for (int i = 0; i < len; i++) {
			Pig pig = pigs.get(i);
	...
	private void checkItemCollisions () {
		...
		for (int i = 0; i < len; i++) {
			...
			if (bob.bounds.overlaps(coin.bounds)) {
                ...
                int lenpig = pigs.size();
		for (int i = 0; i < lenpig; i++) {
			Pig pig = pigs.get(i);
			if (bob.bounds.overlaps(pig.bounds)) {
				pigs.remove(pig);
				lenpig = pigs.size();
				listener.pig();
				score += Pig.PIG_SCORE;
